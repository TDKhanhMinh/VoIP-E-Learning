...
const ManageChats = () => {
      ...
      if (response) {
        ...
        for (const conv of sortedConversations) {
          try {
            ...
            console.error(
              "Error fetching unread count for conversation:",
              error
            );
  ...
  const formatLastMessageTime = (timestamp) => {
    ...
    if (diffInHours < 1) return `${Math.floor(diffInHours * 60)} phút trước`;
    else if (diffInHours < 24) return `${Math.floor(diffInHours)} giờ trước`;
    else return messageTime.toLocaleDateString("vi-VN");
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-slate-900">
      <div className="container px-4 py-1 mx-auto">
        <div className="mb-6">
          <h1 className="text-2xl font-bold text-gray-800 dark:text-gray-100">Hộp thư hỗ trợ</h1>
        </div>

        <div className="overflow-hidden bg-white dark:bg-slate-800 shadow-lg dark:shadow-slate-900/70 rounded-xl">
          <div className="flex h-[calc(100vh-180px)]">
            <div className="w-1/3 border-r border-gray-200 dark:border-slate-600 bg-gray-50 dark:bg-slate-900 flex flex-col">
              <div className="p-4 bg-white dark:bg-slate-700 border-b border-gray-200 dark:border-slate-600">
              ...
              <div className="overflow-y-auto flex-1">
                {isLoading ? (
                  ...
                  <div className="flex flex-col items-center justify-center h-64 p-8 text-gray-500 dark:text-gray-400">
                    <MessageCircle size={48} className="mb-4 text-gray-400 dark:text-gray-500" />
                  ...
                  conversations.map((conv) => {
                    ...
                    return (
                      <div
                        ...
                        className={`p-4 cursor-pointer transition-all duration-200 border-b border-gray-100 dark:border-slate-600 hover:bg-blue-50 dark:hover:bg-slate-800 ${
                          selectedChat?._id === conv._id
                            ? "bg-blue-100 dark:bg-blue-900/30 border-l-4 border-l-blue-500 dark:border-l-blue-400"
                            : ""
                        }`}
                      >
                        <div className="flex items-start space-x-3">
                          <div className="relative">
                            <div className="flex items-center justify-center w-12 h-12 bg-blue-500 dark:bg-blue-600 rounded-full shadow-md dark:shadow-blue-900/50">
                              <User size={20} className="text-white" />
                            </div>
                            {isOnline && (
                              <div className="absolute w-4 h-4 bg-green-500 dark:bg-green-400 border-2 border-white dark:border-slate-900 rounded-full shadow-sm -bottom-1 -right-1"></div>
                            )}
                          </div>
                ...
                <>
                  <div className="p-4 bg-white dark:bg-slate-700 border-b border-gray-200 dark:border-slate-600">
                    <div className="flex items-center space-x-3">
                      <div className="flex items-center justify-center w-12 h-12 bg-blue-500 dark:bg-blue-600 rounded-full shadow-lg dark:shadow-blue-900/50">
                        <User size={20} className="text-white" />
                      </div>
                      <div>
                        <h3 className="text-lg font-semibold text-gray-800 dark:text-gray-100">
                          {userSelected?.full_name || (
                            <div className="h-5 w-32 bg-gray-200 dark:bg-slate-600 rounded animate-pulse"></div>
                          )}
                        </h3>
                        <div className="flex items-center space-x-2">
                          <Circle
                  ...
                  <div className="flex-1 p-4 space-y-2 overflow-y-auto bg-gray-50 dark:bg-slate-900">
                    {isMessageLoading ? (
                      <MessageSkeleton />
                    ) : messages?.length === 0 ? (
                      ...
                      messages?.map((msg, index) => {
                        ...
                        return (
                          ...
                          >
                            <div
                              className={clsx(
                                "px-4 py-3 max-w-xs lg:max-w-md shadow-sm rounded-2xl",
                                isMyMessage
                                  ? "bg-blue-500 dark:bg-blue-600 text-white"
                                  : "bg-white dark:bg-slate-700 text-gray-800 dark:text-gray-100 border border-gray-200 dark:border-slate-600",
                    ...
                    {isTyping && (
                      <div className="flex justify-start">
                        <div className="px-4 py-3 bg-white dark:bg-slate-700 border border-gray-200 dark:border-slate-600 shadow-sm rounded-2xl rounded-bl-md">
                  ...
                  <div className="p-4 bg-white dark:bg-slate-700 border-t border-gray-200 dark:border-slate-600">
                    <div className="flex gap-3">
                      <input
                        ...
                        className="flex-1 px-4 py-3 transition-all border border-gray-300 dark:border-slate-600 rounded-lg outline-none focus:ring-2 focus:ring-blue-500 dark:focus:ring-blue-400 bg-white dark:bg-slate-800 text-gray-900 dark:text-gray-100 placeholder-gray-400 dark:placeholder-gray-500"
                        disabled={!isConnected}
                      />
                      <button
                        onClick={handleSendMessage}
                        disabled={!newMessage.trim() || !isConnected}
                        className="flex items-center gap-2 px-6 py-3 text-white transition-all bg-blue-500 dark:bg-blue-600 rounded-lg shadow-sm dark:shadow-blue-900/50 hover:bg-blue-600 dark:hover:bg-blue-500 disabled:bg-gray-300 dark:disabled:bg-slate-600"
                      >
                        <Send size={18} />
