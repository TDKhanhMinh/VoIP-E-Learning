                ...
                {Object.entries(groupedByClass).map(([className, records]) => {
                    ...
                    return (
                        ...
                        >
                            ...
                            >
                                <div>
                                    <h2 className="font-semibold text-gray-800 text-lg">{className}</h2>
                                    <p className="text-gray-500 text-sm">
                                        Tổng số buổi: {records.length} | Có mặt: {totalPresent} | Muộn: {totalLate} | Vắng: {totalAbsent}
                                    </p>
                                </div>
                                <div className="text-indigo-600 font-medium">
                                    {expandedClass === className ? "▲ Thu gọn" : "▼ Xem chi tiết"}
                                </div>
                            </div>
                                            ...
                                            {records.length > 0 ? (
                                                records.map((r, idx) => (
                                                    <tr key={idx} className="border-t hover:bg-gray-50">
                                                        <td className="p-3 text-gray-700 font-medium">Buổi {r.lesson}</td>
                                                        <td
                                                            className={`p-3 font-semibold ${r.status === "present"
                                                                ? "text-green-600"
                                                                : r.status === "late"
                                                                    ? "text-yellow-600"
                                                                    : "text-red-600"
