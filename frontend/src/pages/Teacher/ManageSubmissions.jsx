...
export default function ManageSubmissions() {
  ...
  const handleSaveGrade = async (data) => {
    try {
      ...
      const grade = await submissionService.updateSubmission(selected._id, data);
      fetchSubmission();
  ...
  const stats = {
    ...
    averageScore: submissions.filter(s => s.score != null).length > 0
      ? (submissions.reduce((acc, s) => acc + (s.score || 0), 0) / submissions.filter(s => s.score != null).length).toFixed(1)
      : "N/A"
  };
      ...
      <div className="max-w-7xl mx-auto space-y-6">
              ...
              <div className="flex-1">
                ...
                <div className="flex flex-wrap gap-4 text-sm text-gray-600">
                  <div className="flex items-center gap-2">
                    ...
                    <span>Hạn nộp: <strong>{assignment?.due_at ? formatDateTime(assignment.due_at) : "—"}</strong></span>
                  </div>
                  <div className="flex items-center gap-2">
                    <FaUsers className="text-gray-400" />
                    <span><strong>{submissions.length}</strong> bài nộp</span>
                  </div>
                </div>
              </div>
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          ...
          <div className="bg-white rounded-2xl shadow-lg p-6 border border-gray-100">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-gray-500 text-sm font-medium">Đã chấm điểm</p>
          ...
          <div className="bg-white rounded-2xl shadow-lg p-6 border border-gray-100">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-3xl font-bold text-orange-600 mt-2">{stats.ungraded}</p>
              </div>
        ...
        <div className="bg-white rounded-2xl shadow-lg p-4 border border-gray-100">
          <div className="flex gap-2 overflow-x-auto">
            ...
            <button
              ...
              className={`px-5 py-2.5 rounded-xl font-medium transition-all whitespace-nowrap ${filterStatus === "graded"
            ...
            <button
              ...
              className={`px-5 py-2.5 rounded-xl font-medium transition-all whitespace-nowrap ${filterStatus === "ungraded"
              ...
              <table className="w-full">
                ...
                <tbody className="divide-y divide-gray-100">
                  {filteredSubmissions.map((s) => {
                    ...
                    const isLate = isLateSubmission(s.createdAt, assignment?.due_at);

                    return (
                      <tr key={s._id} className="hover:bg-blue-50 transition-colors duration-150">
                        ...
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-2">
                            <FaClock className={isLate ? "text-red-500" : "text-gray-400"} />
                            <div>
                        ...
                        <td className="px-6 py-4 text-center">
                          {s.score != null ? (
                            <div className="inline-flex items-center gap-2">
                              <span className={`inline-flex items-center gap-1 px-4 py-2 rounded-xl text-lg font-bold bg-${scoreColor}-100 text-${scoreColor}-700`}>
                        ...
                        <td className="px-6 py-4 text-center">
                          <button
                            ...
                            className={`inline-flex items-center gap-2 px-4 py-2 rounded-xl font-medium shadow-md hover:shadow-lg transition-all duration-200 transform hover:-translate-y-0.5 ${s.graded
